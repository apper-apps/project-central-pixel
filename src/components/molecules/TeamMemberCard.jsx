...
<div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-gray-800 ${
                ...
                backgroundColor: member.status === 'Active' ? '#51D88A' : 
                                member.status === 'Away' ? '#FFB347' : '#B0B0B0'
              }} />
            </div>
            <div>
...
<span className="text-sm font-medium text-gray-200">Current Workload</span>
...
<div className="w-full bg-gray-700 rounded-full h-2">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
<span className="text-sm font-medium text-gray-200">Active Projects</span>
            <span className="text-sm text-gray-300">{member.currentProjects.length}</span>
          </div>
          {member.currentProjects.length > 0 ? (
            <div className="space-y-1">
              {member.currentProjects.slice(0, 2).map((project, index) => (
                <div key={index} className="flex items-center justify-between text-xs">
<span className="text-gray-300 truncate">{project.projectName}</span>
                  <span className="text-gray-400 ml-2">{project.hoursAllocated}h</span>
                </div>
              ))}
...
<div className="text-xs text-gray-400">
        ...
        {/* Stats */}
<div className="grid grid-cols-2 gap-4 mb-4 p-3 bg-gray-700 rounded-lg">
